...
procedure PLP_AddPlayerAsm(sl: TStringList; bLoop: boolean; iType: integer; cInitialTempo: byte);
procedure PLP_AddFreqTable(sl: TStringList);
procedure PLP_AddSongData(sl: TStringList; Song: TSTSong; iTranspose: integer);

implementation

uses SysUtils, GrokUtils;

procedure PLP_Compress(SongIn: TSTSong; SongOut: TSTSong);
var
  i,j: integer;
begin
  SongOut.Clear;
...
begin
  GetAppVersionInfo(s);
  sl.Add('; *****************************************************************************');
  sl.Add('; * Plip Plop Player');
  sl.Add('; *');
  sl.Add('; * By Joffa Smith');
  sl.Add('; * With thanks to Paulie Huges');  
  ...
  sl.Add('');
  sl.Add('; *******************************************************************************');
  sl.Add('; A = Tune number');
  sl.Add('PLAY_TUNE_48:');
  sl.Add('                          DI');
  sl.Add('                          LD   (MUSICSP+1),SP');
  sl.Add('                          EX   AF,AD''                    ; Save tune number');
  sl.Add('                          LD   HL,$fe00                  ; Build interrupt vector table');
  sl.Add('                          LD   DE,$fe00+1');
  sl.Add('                          LD   BC,$100');
  sl.Add('                          LD   (HL),$FF');
  sl.Add('                          LDIR');
  ...
  sl.Add('                          LD   HL,$fff4                 ; save the top 9 bytes');
  sl.Add('                          PUSH HL');
  sl.Add('                          LD   DE,TOP_SAVE');
  sl.Add('                          LD   BC,$0c');
  sl.Add('                          LDIR');
  sl.Add('');
  sl.Add('                          LD   HL,INT_TOP               ; set up top 9 bytes');
  sl.Add('                          POP  DE');
  sl.Add('                          LD   C,$0c');
  sl.Add('                          LDIR');
  sl.Add('                          EX   AF,AF''');
  sl.Add('                          CALL PLAYMUSIC');
  sl.Add('');
  sl.Add('; return here');
  sl.Add('EXIT:                     DI');
  sl.Add('                          LD   HL,INTSR');
  sl.Add('                          LD   (INTJUMP),HL');
  sl.Add('                          LD   HL,TOP_SAVE              ; restore the top 9 bytes');
  sl.Add('                          LD   DE,$fff4');
  sl.Add('                          LD   BC,$0c');
  sl.Add('                          LDIR');
  sl.Add('MUSICSP:                  LD   SP,0                     ; restore stack');
  sl.Add('                          RET');
  sl.Add('');
  sl.Add('PLAYMUSIC:                LD   L,A');
  sl.Add('                          ADD  A');
  sl.Add('                          ADD  L');
  sl.Add('                          ADD  TUNETABLE - PAGE');
  sl.Add('                          LD   L,A');
  sl.Add('                          LD   H,PAGE / 256');
  sl.Add('                          LD   A,(HL)');
  sl.Add('                          LD   (KEYCHECK),A');





  sl.Add('TOP_SAVE:                 DEFB 0,0,0,0,0,0,0,0,0');
  sl.Add('INT_TOP:                  JP   TIMER_INT');
  sl.Add('INTSR:                    EI                            ; in game interrupt routin');
  sl.Add('                          RETI');
  sl.Add('TUNETABLE:                DB   $3C                      ; YES');
  sl.Add('                          DW   TITLE');

  sl.Add('INTJUMP:                  EQU  $FFF5');
...
procedure PLP_AddPatternData(sl: TStringList; Song: TSTSong; iPat: integer; iTranspose: integer);
var
  i: integer;
begin
...
begin
  ...
  begin
    ...
    begin
      PLP_AddPatternData(sl,Song,i,iTranspose); // Add the pattern (from the uncompressed version of the song, as the compressed version does not contain any pattern data)
    end;
  end;
